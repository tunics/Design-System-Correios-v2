(() => {
  const catalogUrl = 'icones/ic-outline.catalog.json';
  const overridesUrl = 'icones/ic-outline.overrides.json';
  const spriteUrl = 'icones/cds-ic-outline.sprite.svg';
  const downloadBaseUrl = 'icones/outline/';
  ...
  function mountInlineSprite(svgText) {
    if (document.getElementById('inline-sprite-outline')) return;

    const wrap = document.createElement('div');
    wrap.id = 'inline-sprite-outline';
  ...
  const buildInlineSvg = (symbol) => {
    if (!symbol) return '';
    return `<svg class="ic-outline" role="img" viewBox="${symbol.viewBox}" xmlns="http://www.w3.org/2000/svg">${symbol.content}</svg>`;
  };
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    const container = document.querySelector('.icones-outline');
    ...
    if (!container || !searchInput || !tagsContainer) {
      console.warn(
        'Não achei .icones-outline, #txt-input-ic ou #busca-tags. Confere teu HTML.',
      );
      return;
    }
    ...
    (catalog?.categories || []).forEach((category) => {
      ...
      (category?.icons || []).forEach((icon) => {
        ...
        svg.setAttribute('class', 'ic-outline');
    ...
    const openAside = (iconId) => {
      ...
      const useCode = `<svg class="ic-outline" role="img"><use xlink:href="#${icon.spriteId}"></use></svg>`;
      if (asideUseCode) {
